...
if [ -z "$security_group_id" ]; then
    echo ">[ERRO] Security group security-group-qa-web não foi criado na VPC $vpc_id"
    exit 1
fi
...
--tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=security-group-qa-web}]' \
